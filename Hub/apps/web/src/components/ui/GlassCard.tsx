...
const toneText: Record<NonNullable<Props['tone']>, string> = {
  default: 'text-[#231F20] dark:text-[#F5F6F4]',
    ...
    >
        ...
        {action && (
          href ? (
            <Link
              ...
              title="Ver detalhes"
              className={twMerge(
                'tap-target pressable focusable grid h-8 w-8 shrink-0 place-items-center rounded-full transition-all duration-200',
                highlight
      ...
      {value && (
        <p
          className={twMerge(
            ...
            highlight ? 'text-[#F5F6F4]' : toneText[tone],
          )}
        >
          {value}
        </p>
      ...
      {(trend || hint) && (
        <div className="mt-2 flex flex-wrap items-center gap-2">
          ...
          {hint && (
            <span className={twMerge('text-xs', highlight ? 'text-[#F5F6F4]/80' : 'text-[#6E6A61] dark:text-[#A8A49C]')}>
              {hint}
            </span>
          )}
        </div>
