    ...
    <div ref={boxRef} className="relative w-64 sm:w-80 md:w-96">
      ...
      {open && query.trim().length >= 2 && (
        <div className="absolute left-0 top-full mt-2 z-50 w-full min-w-[320px] rounded-2xl border border-black/10 dark:border-white/10 bg-[#F5F6F4] dark:bg-[#1A201C] shadow-2xl overflow-hidden">
            ...
            {results.length === 0 ? (
              ...
              results.map((r) => (
                <button
                  ...
                  className="flex w-full items-center gap-3 rounded-xl px-3.5 py-2.5 text-left hover:bg-[#E7EAE5] dark:hover:bg-white/5 transition-colors"
                >
          ...
          <div className="border-t border-black/5 dark:border-white/5 bg-[#E7EAE5]/50 dark:bg-white/5 px-4 py-2 text-[11px] text-[#6E6A61] dark:text-[#A8A49C] text-center">
