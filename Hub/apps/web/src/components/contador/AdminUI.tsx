...
export const fi = 'w-full rounded-2xl border border-black/10 dark:border-white/10 bg-[#F5F6F4] dark:bg-[#121614] px-3.5 py-2.5 text-sm text-[#231F20] dark:text-[#F5F6F4] outline-none transition-colors focus:border-[#2F4A3C] focus:ring-2 focus:ring-[#DFFFAE]';
...
export function Section({
  ...
  return (
    <section className={`rounded-3xl border border-black/5 dark:border-white/10 surface-panel p-6 sm:p-8 shadow-sm ${fullWidth ? 'lg:col-span-2' : ''}`}>
      <div className="mb-6 flex items-start gap-3.5">
        ...
        <div>
          <h2 className="font-serif font-bold text-lg text-[#231F20] dark:text-[#F5F6F4]">{title}</h2>
...
export function Toggle({ label, defaultOn = false }: { label: string; defaultOn?: boolean }) {
  ...
  return (
    <label className="flex cursor-pointer items-center justify-between py-3 border-b border-black/5 dark:border-white/10 last:border-b-0">
      <span className="text-sm font-medium text-[#231F20] dark:text-[#F5F6F4]">{label}</span>
    ...
    <div>
      ...
      <div className="relative mt-1.5">
        <input readOnly value={value}
          className={`${fi} pr-10 font-mono text-xs text-[#6E6A61] dark:text-[#A8A49C] bg-[#E7EAE5]/40 dark:bg-[#1A201C]/40 cursor-default`} />
