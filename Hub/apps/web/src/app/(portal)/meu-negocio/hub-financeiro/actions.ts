...
import { category, recurringExpense } from '@hexxa/db/schema';
  ...
  await withTenant(ctx.companyId, async (tx) => {
    ...
    for (let i = 0; i < total; i++) {
      ...
      await tx.execute(sql`
        INSERT INTO financial_entry (company_id, type, description, amount, due_date, reference_month, status, source, category_id, receipt_base64, receipt_filename, receipt_mime_type)
        VALUES (${ctx.companyId}, ${typeStr}, ${title}, ${data.valor.toString()}, ${dueDateStr}, ${refMonth}, 'PENDING', 'MANUAL', ${categoryId}, ${receiptBase64}, ${receiptFilename}, ${receiptMimeType})
      `);
    }
  });
