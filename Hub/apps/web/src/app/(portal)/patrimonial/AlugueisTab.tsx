...
const lbl = 'text-caption font-bold text-ink-soft uppercase tracking-wide';
...
const STATUS_CFG: Record<string, { label: string; cls: string; icon: React.FC<{ className?: string }> }> = {
  PENDING: { label: 'Em aberto', cls: 'bg-amber-500/10 text-amber-600 dark:text-amber-400', icon: Clock },
  PAID: { label: 'Recebido', cls: 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400', icon: CheckCircle2 },
  OVERDUE: { label: 'Vencido', cls: 'bg-red-500/10 text-red-600 dark:text-red-400', icon: AlertTriangle },
  CANCELED: { label: 'Cancelado', cls: 'bg-surface-card text-ink-soft', icon: XCircle },
};
...
const LEASE_STATUS_CLASS: Record<LeaseRow['status'], string> = {
  DRAFT: 'bg-surface-card text-ink-soft border border-black/5 dark:border-white/5',
  PENDING_SIGNATURE: 'bg-amber-500/10 text-amber-600 dark:text-amber-400 border border-amber-500/20',
  ACTIVE: 'bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20',
  ENDED: 'bg-surface-card text-ink-soft border border-black/5 dark:border-white/5',
  CANCELED: 'bg-surface-card text-ink-soft border border-black/5 dark:border-white/5',
};
...
function NovoAluguelForm({ properties, onClose, onDone }: { properties: PropertyRow[]; onClose: () => void; onDone: () => void }) {
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-3xl border border-black/5 dark:border-white/5 bg-surface-card shadow-(--elev-inset) p-6 space-y-4 animate-in fade-in">
      <div className="flex items-center justify-between border-b border-black/5 dark:border-white/10 pb-3">
        <p className="font-serif font-bold text-base text-ink">Novo Contrato de Aluguel</p>
        <button type="button" onClick={onClose} className="tap-target pressable focusable rounded-full p-1.5 text-ink-soft hover:bg-black/5 dark:hover:bg-white/10 transition-colors">
          <X className="h-4 w-4" />
        </button>
      </div>
      ...
      {disponiveis.length === 0 ? (
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div className="sm:col-span-2">
            ...
            <p className="mt-1 text-[11px] text-ink-soft">Deixe em branco pra um contrato por prazo indeterminado — geramos 24 meses de lançamentos e renovamos conforme o tempo passa.</p>
          </div>
        </div>
      )}
      ...
      {disponiveis.length > 0 && (
        <div className="flex gap-2 pt-2">
          <button type="submit" disabled={submitting} className="inline-flex items-center gap-2 rounded-full bg-hexxa-forest hover:brightness-110 active:scale-95 px-5 py-2.5 text-xs font-bold text-hexxa-lime shadow-(--elev-1) transition-all disabled:opacity-60">
            {submitting ? <Loader2 className="h-4 w-4 animate-spin" /> : <Plus className="h-4 w-4" />} Registrar Contrato
          </button>
          <button type="button" onClick={onClose} className="rounded-full border border-black/5 dark:border-white/5 bg-surface-card px-4 py-2.5 text-xs font-bold text-ink-soft hover:text-ink shadow-(--elev-1) transition-colors">
            Cancelar
          </button>
        </div>
      )}
    ...
    <Card level={1} className="overflow-hidden">
      <div className="p-6 space-y-4">
          ...
          <div>
            <div className="flex items-center gap-2 flex-wrap">
              <h3 className="font-serif font-bold text-base text-ink flex items-center gap-1.5">
                <Home className="h-4 w-4 text-hexxa-forest dark:text-hexxa-lime" /> {lease.propertyName}
              </h3>
            ...
            <p className="text-xs text-ink-soft mt-1">
              Locatário: <strong className="text-ink">{lease.lesseeName}</strong> · Índice {lease.indexType === 'IPCA' ? 'IPCA' : 'IGP-M'}
            </p>
          </div>
          <div className="text-right">
            <p className="font-serif tabular font-bold text-xl text-ink">{BRL.format(lease.monthlyRent)}<span className="text-xs font-sans font-normal text-ink-soft">/mês</span></p>
            <p className="text-[11px] text-ink-soft">Vigência: {fmtDate(lease.startDate)} — {lease.endDate ? fmtDate(lease.endDate) : 'indeterminado'}</p>
          </div>
        </div>
        ...
        <p className="text-[11px] text-ink-soft -mt-2">
          Resultado contábil após depreciação (não é saída de caixa): <strong className="text-ink font-serif tabular">{BRL.format(resultadoContabil)}</strong>/mês.
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 pt-2 border-t border-black/5 dark:border-white/10">
          ...
          <div className="flex flex-wrap items-center gap-2">
            {lease.hasPdf && (
              <button type="button" onClick={handleVerPdf} className="inline-flex items-center gap-1.5 rounded-full border border-black/5 dark:border-white/5 bg-surface-card px-3.5 py-1.5 text-xs font-bold text-ink-soft hover:text-ink shadow-(--elev-1) transition-colors">
                Ver PDF
              </button>
            )}
            {lease.status === 'ACTIVE' && (
              <button type="button" onClick={() => setShowReajuste((v) => !v)} className="inline-flex items-center gap-1.5 rounded-full border border-black/5 dark:border-white/5 bg-surface-card px-3.5 py-1.5 text-xs font-bold text-ink-soft hover:text-ink shadow-(--elev-1) transition-colors">
                <Percent className="h-3.5 w-3.5" /> Reajustar
              </button>
            )}
            {(lease.status === 'ACTIVE' || lease.status === 'PENDING_SIGNATURE') && (
              <button type="button" onClick={handleEncerrar} disabled={busy} className="inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-bold text-red-600 hover:bg-red-500/10 disabled:opacity-50 transition-colors">
        ...
        <button type="button" onClick={toggleExpand} className="flex w-full items-center justify-between text-xs font-bold text-hexxa-forest dark:text-hexxa-lime hover:underline pt-1">
        ...
        <div className="border-t border-black/5 dark:border-white/10 divide-y divide-black/5 dark:divide-white/10">
          {loadingPayments ? (
            ...
            payments.map((p) => {
              ...
              return (
                <div key={p.id} className="flex flex-wrap items-center gap-3 px-6 py-3.5 hover:bg-surface-card-hover transition-colors">
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-xs font-bold text-ink">{p.description}</p>
                    <p className="text-[11px] text-ink-soft">Vence {fmtDate(p.dueDate)}{p.paidAt ? ` · recebido em ${fmtDate(p.paidAt)}` : ''}</p>
                  </div>
                  ...
                  {p.hasReceipt && (
                    <button type="button" onClick={() => handleVerComprovante(p.id)} className="inline-flex items-center gap-1 rounded-full bg-surface-card border border-black/5 dark:border-white/5 shadow-(--elev-1) px-2.5 py-1 text-[11px] font-bold text-ink-soft hover:text-ink transition-colors">
                      <Paperclip className="h-3 w-3" /> Comprovante
                    </button>
                  )}
                  {p.status === 'PENDING' && (
                    <button type="button" onClick={() => handleMarcarPago(p.id)} disabled={busy} className="inline-flex items-center gap-1 rounded-full bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20 px-2.5 py-1 text-[11px] font-bold hover:bg-emerald-500/20 transition-colors disabled:opacity-50">
            ...
            <p className="py-8 text-center text-xs text-ink-soft">Nenhum lançamento encontrado.</p>
          )}
        </div>
      )}
    </Card>
  ...
  return (
    <div className="space-y-6 animate-in fade-in">
      ...
      <div className="flex items-center justify-between">
        ...
        <button type="button" onClick={() => setShowForm((v) => !v)} className="inline-flex items-center gap-2 rounded-full bg-hexxa-forest hover:brightness-110 active:scale-95 px-5 py-2.5 text-xs font-bold text-hexxa-lime shadow-(--elev-1) transition-all">
      ...
      <div className="space-y-4">
        {leases.length === 0 ? (
          <p className="text-sm text-ink-soft py-12 text-center">Nenhum contrato de aluguel cadastrado ainda.</p>
        ) : (
          leases.map((l) => (
