    ...
    <form onSubmit={handleSave} className="space-y-6">
      ...
      <div className="divide-y divide-black/5 dark:divide-white/10 border border-black/5 dark:border-white/10 rounded-2xl overflow-hidden bg-surface-card shadow-(--elev-inset)">
        {QUICK_ACTIONS_CATALOG.map((action) => {
          ...
          return (
            <div key={action.id} className="flex items-center justify-between p-4 gap-4">
              <div className="flex items-center gap-3 min-w-0">
                <span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-hexxa-forest/15 text-hexxa-forest dark:bg-hexxa-lime/15 dark:text-hexxa-lime">
                  <Icon className="h-4 w-4" />
                </span>
                <div className="min-w-0">
                  <p className="text-xs sm:text-sm font-bold text-ink truncate">{action.label}</p>
                  <p className="text-[11px] text-ink-soft truncate">{action.sub}</p>
                </div>
              </div>
              <button
                type="button"
                ...
                className={`relative inline-flex h-6 w-11 shrink-0 cursor-pointer rounded-full border-2 border-transparent transition-colors duration-200 ease-in-out ${
                  checked ? 'bg-hexxa-forest' : 'bg-black/10 dark:bg-white/10'
                }`}
              >
                <span
      ...
      <div className="pt-2">
        <button
          ...
          className="rounded-full bg-hexxa-forest hover:brightness-110 px-6 py-2.5 text-xs font-bold text-hexxa-lime shadow-(--elev-1) transition-all active:scale-95 disabled:opacity-50"
        >
