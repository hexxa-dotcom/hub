...
export function NotasFiscaisTable({ initial }: { initial: NotaFiscal[] }) {
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 animate-in fade-in">
      <div>
        <h1 className="font-serif font-bold text-2xl sm:text-3xl tracking-tight text-[#231F20] dark:text-[#F5F6F4]">Notas Fiscais</h1>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        {[
          ...
          { label: 'Notas emitidas', value: String(initial.filter(n => n.status === 'ISSUED').length), cls: 'text-[#231F20] dark:text-[#F5F6F4]' },
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-2 rounded-full border border-black/10 dark:border-white/10 bg-[#F5F6F4] dark:bg-[#121614] px-4 py-2 shadow-xs">
          <Search className="h-4 w-4 text-[#6E6A61] dark:text-[#A8A49C]" />
          <input value={search} onChange={e => setSearch(e.target.value)}
            placeholder="Buscar cliente, tomador, NF…" className="w-48 sm:w-64 bg-transparent text-xs sm:text-sm text-[#231F20] dark:text-[#F5F6F4] outline-none" />
        </div>
      ...
      <div className="overflow-hidden rounded-3xl border border-black/5 dark:border-white/10 surface-panel shadow-sm">
            ...
            {filtered.map(n => {
              ...
              return (
                <tr key={n.id} className="hover:bg-black/5 dark:hover:bg-white/5 transition-colors">
                  ...
                  <td className="px-3 py-3.5 font-bold text-[#231F20] dark:text-[#F5F6F4]">
                  ...
                  <td className="px-3 py-3.5 text-right font-bold text-[#231F20] dark:text-[#F5F6F4]">{BRL.format(n.valor)}</td>
        ...
        {filtered.length === 0 && (
          <div className="py-12 text-center text-xs sm:text-sm text-[#6E6A61] dark:text-[#A8A49C] bg-[#F5F6F4] dark:bg-[#121614]">Nenhuma nota encontrada.</div>
        )}
      </div>
    </div>
  );
