...
import { Spinner, LockOpen, WarningCircle, CheckCircle } from '@phosphor-icons/react';
...
export function ManualAuthorization() {
  ...
  return (
    <div className="relative">
      ...
      >
        <LockOpen className="h-4 w-4" /> Liberação Manual
      </button>
          ...
          <form onSubmit={handleAuthorize} className="space-y-3">
            {message && (
              <div className={`p-2 text-xs rounded-xl flex items-start gap-1.5 ${message.type === 'error' ? 'bg-red-50 text-red-600 dark:bg-red-900/30' : 'bg-emerald-50 text-emerald-600 dark:bg-emerald-900/30'}`}>
                {message.type === 'error' ? <WarningCircle className="w-4 h-4 shrink-0" /> : <CheckCircle className="w-4 h-4 shrink-0" />}
            ...
            <div className="flex justify-end gap-2">
              ...
              >
                {isLoading ? <Spinner className="w-4 h-4 animate-spin" /> : 'Autorizar'}
              </button>
            </div>
          </form>
        </div>
