    ...
    <div className="mx-auto max-w-5xl space-y-6 animate-in fade-in">
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <div className="rounded-3xl border border-black/5 dark:border-white/10 surface-panel p-6 sm:p-8 shadow-sm">
          <h2 className="font-serif font-bold text-base text-[#231F20] dark:text-[#F5F6F4] mb-4 flex items-center gap-2">
          ...
          {history[0] ? (
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="bg-[#F5F6F4] dark:bg-[#121614] border border-black/5 dark:border-white/10 p-4 rounded-2xl">
                  <p className="text-[10px] font-bold text-[#6E6A61] dark:text-[#A8A49C] uppercase tracking-wider mb-1">RBA 12</p>
                  <p className="font-bold text-[#231F20] dark:text-[#F5F6F4] text-lg">
              ...
              <p className="text-xs text-[#6E6A61] dark:text-[#A8A49C] mt-2">
                Enquadramento: <strong className="text-[#231F20] dark:text-[#F5F6F4]">{history[0].taxBracket}</strong>
              </p>
            </div>
          ) : (
      ...
      <div className="rounded-3xl border border-black/5 dark:border-white/10 surface-panel p-6 sm:p-8 shadow-sm">
        <h2 className="font-serif font-bold text-base text-[#231F20] dark:text-[#F5F6F4] mb-1 flex items-center gap-2">
      ...
      <div className="rounded-3xl border border-black/5 dark:border-white/10 surface-panel shadow-sm overflow-hidden">
        <div className="p-5 border-b border-black/5 dark:border-white/10">
          <h2 className="font-serif font-bold text-sm text-[#231F20] dark:text-[#F5F6F4]">Histórico de PGDAS</h2>
        </div>
          ...
          <tbody className="divide-y divide-black/5 dark:divide-white/10 bg-[#F5F6F4] dark:bg-[#121614]">
            {history.length > 0 ? history.map((item) => (
              <tr key={item.id} className="hover:bg-black/5 dark:hover:bg-white/5 transition-colors">
                <td className="px-5 py-3 font-medium text-[#231F20] dark:text-[#F5F6F4]">{item.referenceMonth}</td>
                <td className="px-5 py-3 text-right text-[#6E6A61] dark:text-[#A8A49C]">{BRL.format(parseFloat(item.rba12))}</td>
                <td className="px-5 py-3 text-right font-bold text-[#231F20] dark:text-[#F5F6F4]">{item.effectiveRate}%</td>
