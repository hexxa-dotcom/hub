...
async function getPlanos(): Promise<PlanoOpcao[]> {
  const db = getDb();
  const rows = await db.select({ nome: plan.name, valor: plan.monthlyValue }).from(plan);
...
export default async function AdminContratosPage() {
  const [clientes, planos, historico] = await Promise.all([getClientes(), getPlanos(), getHistorico()]);
